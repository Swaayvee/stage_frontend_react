...
function migrateStateRoles(currentState) {
  ...
  if ((currentState.meta.seedVersion || 0) < 9) {
    const seed = createSeedState();
    currentState.livreurs.forEach((courier) => {
      const seeded = seed.livreurs.find((item) => item._id === courier._id);
      courier.partagePositionActif = seeded?.partagePositionActif === true;
      courier.positionActualiseeLe = courier.partagePositionActif ? (seeded?.positionActualiseeLe || nowIso()) : null;
    });
    currentState.vendeurs.forEach((seller) => {
      seller.coordonnees ||= seed.vendeurs.find((item) => item._id === seller._id)?.coordonnees || null;
    });
    currentState.livraisons.forEach((delivery) => {
      delivery.coordonneesLivraison ||= seed.livraisons.find((item) => item._id === delivery._id)?.coordonneesLivraison || null;
    });
    currentState.meta.seedVersion = 9;
  }
  return currentState;
}
...
export function replaceStateFromServer(nextState) {
  if (!nextState || !Array.isArray(nextState.comptes)) return { ok: false, error: "État serveur invalide." };
  state = migrateStateRoles(clone(nextState));
  persist();
...
export function hydrateAuthenticatedAccount(serverAccount) {
  loadState();
  if (!serverAccount?._id || !serverAccount?.email || !["vendeur", "livreur", "manager", "super_manager"].includes(serverAccount.role)) {
    return { ok: false, error: "Données de compte serveur invalides." };
  }
  let compte = state.comptes.find((item) => item._id === serverAccount._id);
  if (!compte) {
    compte = {
      _id: serverAccount._id,
      email: serverAccount.email.trim().toLowerCase(),
    ...
    compte.email = serverAccount.email.trim().toLowerCase();
    compte.role = serverAccount.role;
    compte.statutCompte = "actif";
  }
  const profile = serverAccount.profile || {};
  const fallbackName = serverAccount.email.split("@")[0].replace(/[._-]+/g, " ");
  if (serverAccount.role === "livreur" && !state.livreurs.some((item) => item.compteId === compte._id)) {
    state.livreurs.push({
      _id: uid("liv"),
      compteId: compte._id,
      nom: profile.nom || fallbackName,
      telephone: profile.telephone || "",
      adresse: profile.adresse || "",
      ville: profile.ville || "",
      departement: profile.departement || "",
      typeVehicule: profile.typeVehicule || "Non renseigné",
      statutOperationnel: "indisponible",
      rayonRechercheKm: 5,
      coordonnees: null,
      partagePositionActif: false,
      positionActualiseeLe: null,
      dateCreation: nowIso(),
    });
  }
  if (serverAccount.role === "vendeur" && !state.vendeurs.some((item) => item.compteId === compte._id)) {
    const vendeurId = uid("ven");
    state.vendeurs.push({
      _id: vendeurId,
      compteId: compte._id,
      raisonSociale: profile.raisonSociale || profile.nom || fallbackName,
      telephone: profile.telephone || "",
      adresse: profile.adresse || "",
      ville: profile.ville || "",
      departement: profile.departement || "",
      typeCommerce: profile.typeCommerce || "",
      dateCreation: nowIso(),
    });
    state.abonnements.push({
      _id: uid("abo"), vendeurId, montantMensuel: TARIF_ABONNEMENT_MENSUEL,
      statut: "actif", dateDebut: nowIso(), dateFin: null,
    });
  }
  persist();
...
export function displayNameForCompte(compteId) {
  ...
  if (p.type === "manager") return p.nom || "Manager";
...
export function updateLivreurCoords(livreurCompteId, coords) {
  ...
  liv.partagePositionActif = true;
  liv.positionActualiseeLe = nowIso();
  persist();
  return { ok: true };
}

export function stopLivreurLocationSharing(livreurCompteId) {
  loadState();
  const livreur = state.livreurs.find((item) => item.compteId === livreurCompteId);
  if (!livreur) return { ok: false, error: "Livreur introuvable." };
  livreur.partagePositionActif = false;
  livreur.positionActualiseeLe = null;
  persist();
  return { ok: true };
}
