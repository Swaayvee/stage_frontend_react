...
async function createIndexes(db) {
  await Promise.all([
    ...
    db.collection("applicationDocuments").createIndex({ applicationId: 1 }),
  ]);
}

function validatedDocument(document) {
  const encoded = String(document?.dataBase64 || "").replace(/\s/g, "");
  if (!encoded || !/^[A-Za-z0-9+/]+={0,2}$/.test(encoded))
    throw new Error("Le contenu d’un justificatif est invalide.");
  const data = Buffer.from(encoded, "base64");
  if (!data.length || data.length > 10_000_000)
    throw new Error("Chaque justificatif doit peser entre 1 octet et 10 Mo.");
  if (Number(document?.taille) !== data.length)
    throw new Error("La taille d’un justificatif ne correspond pas au fichier envoyé.");
  const isPdf = data.subarray(0, 4).toString() === "%PDF";
  ...
  if (!type) throw new Error("Seuls les fichiers PDF, PNG, JPEG et WebP sont autorisés.");
  return {
    _id: randomUUID(),
    nom: String(document?.nom || "justificatif").replace(/[\\/\r\n]/g, "_").slice(0, 180),
    type,
    taille: data.length,
    data,
  };
...
export async function storeApplicationDocuments(applicationId, documents = []) {
  const db = await initializeDatabase();
  const validated = documents.slice(0, 10).map(validatedDocument);
  if (validated.length) {
    await db.collection("applicationDocuments").insertMany(validated.map((document) => ({
      ...document,
      applicationId,
      dateCreation: new Date(),
    })));
  }
  return validated.map(({ _id, nom, type, taille }) => ({
    _id,
    nom,
    type,
    taille,
    url: `/api/registration-applications/${encodeURIComponent(applicationId)}/documents/${encodeURIComponent(_id)}`,
  }));
}

export async function removeApplicationDocuments(applicationId) {
  const db = await initializeDatabase();
  await db.collection("applicationDocuments").deleteMany({ applicationId });
}

export async function getApplicationDocument(applicationId, documentId, actor) {
  const db = await initializeDatabase();
  const application = await db.collection("applications").findOne({ _id: applicationId });
  if (!application) return null;
  if (actor.role === "manager") {
    const manager = await db.collection("gestionnaires").findOne({ compteId: actor.sub });
    const assigned = manager && (application.managerIds || []).includes(manager._id);
    if (!assigned && !application.fallbackTousManagers) return null;
  } else if (actor.role !== "super_manager") {
    return null;
  }
  return db.collection("applicationDocuments").findOne({ _id: documentId, applicationId });
}
